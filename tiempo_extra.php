...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($num_ficha) || empty($semana_inicio) || empty($motivo) || $horas_extra <= 0) {
        ...
        if ($check_result->num_rows == 0) {
            ...
            // Insertar la solicitud de tiempo extra
            $insert_query = "INSERT INTO tiempo_extra (num_ficha, semana_inicio, horas_extra, motivo, supervisor_id) 
                           VALUES (?, ?, ?, ?, ?)";
            $insert_stmt = $conn->prepare($insert_query);
            $insert_stmt->bind_param("ssisi", $num_ficha, $semana_inicio, $horas_extra, $motivo, $user_id);
            ...
            if ($insert_stmt->execute()) {
                ...
                $detalles = "Empleado: $num_ficha, Semana: $semana_inicio, Horas: $horas_extra";
